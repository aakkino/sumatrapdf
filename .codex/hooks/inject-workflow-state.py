...
def _codex_dispatch_context(root: Path) -> str:
    """Render persisted implement/check overrides without blocking the hook.

    This hook runs before the main session's native Codex spawn. A resolver
    failure is therefore useful context, but must never prevent unrelated
    prompts from receiving their normal workflow breadcrumb.
    """
    scripts_dir = root / ".trellis" / "scripts"
    if str(scripts_dir) not in sys.path:
        sys.path.insert(0, str(scripts_dir))
    try:
        from dispatch_routes import (  # type: ignore[import-not-found]
            DISPATCH_LANE_ORDER,
        ...
        return (
            "<trellis-dispatch>\n"
            f"Codex implement/check dispatch is BLOCKED: {exc}. "
            "Do not spawn trellis-implement or trellis-check until this "
            "persistent configuration is repaired. Other prompts may continue.\n"
            "</trellis-dispatch>"
        )

    lines = [
        "<trellis-dispatch>",
        "Resolved persistent Codex dispatch lanes. Immediately before a native "
        "spawn, select the classified lane and pass its exact model and "
        "reasoning_effort override:",
    ]
    blocked = False
    for role, lane in DISPATCH_LANE_ORDER:
        try:
            item = resolve_dispatch_lane(root, role, lane)
        except Exception as exc:
            blocked = True
            lines.append(f"- dispatch:{role}:{lane}: BLOCKED ({exc})")
        else:
            lines.append(
                f"- {item.scope}: {item.route} ({item.model} / {item.effort})"
            )
    if blocked:
        lines.append(
            "Only lanes marked BLOCKED are prohibited until repaired. "
            "Other prompts may continue, and valid dispatch lanes remain available."
        )
    lines.append("</trellis-dispatch>")
    return "\n".join(lines)
...
def main() -> int:
    ...
    if platform == "codex":
        ...
        if (
            task is not None
            and task[1] == "in_progress"
            and _resolve_codex_dispatch_mode(config) == "auto"
        ):
            parts.append(_codex_dispatch_context(root))
