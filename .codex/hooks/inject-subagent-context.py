...
Injects task-specific context when Trellis sub-agents are spawned.
...
- check.md        - Paired-check and final-gate contract
...
AGENT_FINAL_CHECK = "trellis-final-check"
AGENT_RESEARCH = "trellis-research"

# Agents that require a task directory
AGENTS_REQUIRE_TASK = (AGENT_IMPLEMENT, AGENT_CHECK, AGENT_FINAL_CHECK)
# All supported agents
AGENTS_ALL = (AGENT_IMPLEMENT, AGENT_CHECK, AGENT_FINAL_CHECK, AGENT_RESEARCH)
...
def get_check_context(repo_root: str, task_dir: str) -> str:
    ...
    for filename, label, reason in (
        ("check.md", "Check Contract", "Paired-check authority and command budget"),
        ("baseline.json", "Protected Baseline", "Protected pre-existing changes"),
        ("evidence.jsonl", "Recorded Evidence", "Prior validation evidence"),
    ):
...
def get_final_check_context(repo_root: str, task_dir: str) -> str:
    """Dedicated, read-only final-gate context with frozen-candidate state."""
    limits = _get_limits(repo_root)
    budget = _Budget(limits["max_total_bytes"])
    context_parts: list[str] = []
    manifest = get_agent_context(repo_root, task_dir, "check", limits, budget)
    if manifest:
    ...
    for filename, label, reason in (
        ...
        ("implement.md", "Execution Plan", "Reviewed implementation contract"),
        ("check.md", "Check Contract", "Final-gate command budget"),
        ("baseline.json", "Protected Baseline", "Protected pre-existing changes"),
        ("evidence.jsonl", "Recorded Evidence", "Candidate-bound validation evidence"),
        ("candidate.json", "Frozen Candidate", "Frozen candidate digest"),
    ):
        block = _materialize_artifact(
            repo_root,
            f"{task_dir}/{filename}",
...
3. **Implement prescribed unit** - Make only the reviewed changes in the named unit
4. **Inspect the diff** - Report the prescribed actions completed, deviations, and blockers

## Important Constraints

- Do NOT execute tests, lint, type-check, build, or validation commands
- Only run a formatter or generator explicitly authorized by the implementation contract
- Do NOT make product, architecture, validation-scope, or ownership decisions
- Do NOT execute git commit; report changed files and prescribed-action handoff only"""
...
This invocation is the one paired check stage for a completed implementation
unit. Audit ownership and requirements first. The check contract defaults to
T0 and grants no command authority beyond its listed paired commands.
...
1. **Audit ownership and requirements** - Check the declared unit and protected baseline
2. **Self-fix only declared scope** - Use only allowed paths and issue classes
3. **Run listed evidence commands only** - Respect tier and maxRuns

## Important Constraints

- Do not run tests, lint, type-check, build, or a broader suite unless explicitly listed
- After an in-scope self-fix, rerun only invalidated evidence once unless check.md permits more
- Do not hand in-scope findings back to implement or create another role stage
- Escalate only a missing product decision, new authority, protected-path drift, or outside scope"""


def build_final_check_prompt(original_prompt: str, context: str) -> str:
    return f"""<!-- trellis-hook-injected -->
# Final Check Agent Task

You are the read-only `trellis-final-check` role. Do not edit files, fix
findings, or create another role stage. Verify the frozen candidate digest,
protected baseline, and candidate-bound evidence. Run only commands listed in
the finalGate budget; its default is zero commands. Return exactly `ready` or
...
def build_codex_subagent_context(
    ...
    if subagent_type == AGENT_FINAL_CHECK:
        return f"""<!-- trellis-hook-injected -->
# Trellis Native Final Check Subagent

You are the read-only `{subagent_type}` role. Do not edit files or fix
findings. Verify the frozen candidate digest, protected baseline, and
candidate-bound evidence. Run only check.md finalGate commands, which default
to zero. Return exactly `ready` or `blocked` with evidence.
...
def _handle_codex_subagent_start(input_data: dict) -> None:
    ...
    elif subagent_type == AGENT_FINAL_CHECK:
        context = get_final_check_context(repo_root, task_dir)
    else:
...
def main():
    ...
    if subagent_type == AGENT_IMPLEMENT:
        ...
        assert task_dir is not None  # validated above
        context = get_final_check_context(repo_root, task_dir)
        new_prompt = build_final_check_prompt(original_prompt, context)
