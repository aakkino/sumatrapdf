...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    contract_errors = validate_task_contract(full_path, repo_root)
    if contract_errors:
        print(colored("Error: task planning contract is invalid", Colors.RED))
        for error in contract_errors:
            print(f"  - {error}")
        return 1

    baseline_errors = capture_baseline(full_path, repo_root)
    if baseline_errors:
        print(colored("Error: unable to capture protected baseline", Colors.RED))
...
def _resolve_runtime_task(task_input: str) -> tuple[object | None, object | None]:
    repo_root = get_repo_root()
...
def cmd_validate_stage(args: argparse.Namespace) -> int:
    """Validate protected paths and declared ownership for one unit."""
    repo_root, task_dir = _resolve_runtime_task(args.dir)
    if repo_root is None or task_dir is None:
        return 1
    errors = validate_protected_baseline(task_dir, repo_root)
    errors.extend(validate_unit_ownership(task_dir, repo_root, args.unit))
    if errors:
        print(colored("Error: stage validation failed", Colors.RED))
        for error in errors:
            print(f"  - {error}")
        return 1
    print(colored(f"✓ Stage validation passed for unit '{args.unit}'", Colors.GREEN))
    return 0


def cmd_freeze_candidate(args: argparse.Namespace) -> int:
    """Freeze the current candidate digest for a read-only final gate."""
    repo_root, task_dir = _resolve_runtime_task(args.dir)
    if repo_root is None or task_dir is None:
        return 1
    digest, errors = freeze_candidate(task_dir, repo_root)
    if errors:
        print(colored("Error: could not freeze candidate", Colors.RED))
        for error in errors:
            print(f"  - {error}")
        return 1
    print(colored(f"✓ Candidate frozen: {digest}", Colors.GREEN))
    return 0


def cmd_record_evidence(args: argparse.Namespace) -> int:
    """Record an already-run, contract-authorized validation result."""
    repo_root, task_dir = _resolve_runtime_task(args.dir)
    if repo_root is None or task_dir is None:
        return 1
    errors = record_evidence(
        task_dir,
        repo_root,
        stage=args.stage,
        unit_id=args.unit,
    ...
    if errors:
        print(colored("Error: evidence was not recorded", Colors.RED))
        for error in errors:
            print(f"  - {error}")
        return 1
    print(colored(f"✓ Recorded {args.stage} evidence '{args.command_id}'", Colors.GREEN))
    return 0


def cmd_final_check(args: argparse.Namespace) -> int:
    """Return the read-only readiness decision for a frozen candidate."""
    repo_root, task_dir = _resolve_runtime_task(args.dir)
    if repo_root is None or task_dir is None:
        return 1
    decision, errors = final_gate_decision(task_dir, repo_root)
    if decision == "ready":
        print("ready")
        return 0
    print("blocked")
...
def main() -> int:
    ...
    # machine-checked execution contract
    p_stage = subparsers.add_parser("validate-stage", help="Validate one implementation unit's ownership")
    p_stage.add_argument("dir", help="Task directory")
    p_stage.add_argument("unit", help="Implementation unit id")

    p_freeze = subparsers.add_parser("freeze-candidate", help="Freeze the current candidate digest")
    p_freeze.add_argument("dir", help="Task directory")

    p_evidence = subparsers.add_parser("record-evidence", help="Record an authorized validation result")
    p_evidence.add_argument("dir", help="Task directory")
    p_evidence.add_argument("stage", choices=("paired", "final"), help="Evidence stage")
    p_evidence.add_argument("unit", help="Implementation unit id, or final-gate for final evidence")
    p_evidence.add_argument("command_id", help="Authorized check.md command id")
    p_evidence.add_argument("result", choices=("pass", "fail"), help="Command result")

    p_final = subparsers.add_parser("final-check", help="Read-only final readiness decision")
    p_final.add_argument("dir", help="Task directory")
    ...
    commands = {
        ...
        "freeze-candidate": cmd_freeze_candidate,
