...
from .task_contract import validate_task_contract
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    total_errors = 0
    contract_errors = validate_task_contract(target_dir, repo_root)
    for error in contract_errors:
        print(f"  {colored(error, Colors.RED)}")
    total_errors += len(contract_errors)
