...
- Replace every `TBD` marker before starting implementation.
- Keep this document focused on requirements, constraints, and acceptance criteria.
"""


def _default_design_content() -> str:
...
def _default_implement_content() -> str:
...
def _default_check_content() -> str:
...
```json
{
  ...
  "selfFix": {"allowedPaths": [], "allowedIssueClasses": [], "maxRounds": 0},
  "paired": {"maxTier": "T0", "commands": []},
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    for filename, content in (
        ("design.md", _default_design_content()),
        ("implement.md", _default_implement_content()),
        ("check.md", _default_check_content()),
    ):
        path = task_dir / filename
        if not path.exists():
            path.write_text(content, encoding="utf-8")

    # All task plans carry manifest skeletons. A task cannot enter
    # implementation until planning replaces the seed rows with real context.
