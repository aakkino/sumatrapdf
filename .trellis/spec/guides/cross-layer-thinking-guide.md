...
## Generated Harness Export Completeness

A generated harness is a contract across CLI dispatch, platform
configurators, shared templates, root-file writers, and runtime-created state.
Directory names mentioned in a request are not a reliable export boundary.

### Checklist: Before Exporting A Generated Harness

- [ ] Pin the exact package version and trace every writer reachable from the
      requested `init` command
- [ ] Run that version in a fresh empty directory with the same platform flags
      and compare its complete top-level output with the writer map
- [ ] Map every generated path to its writer, current project source, export
      destination, and one explicit disposition: copy, regenerate,
      placeholder, or exclude
- [ ] Trace the functions that actually enumerate/copy templates; do not infer
      active injection merely from files present under `dist/templates/`
- [ ] Distinguish behavior/configuration files, managed metadata, empty
      directories, default task/workspace skeletons, and runtime history
- [ ] Validate file-set and hash parity for copied surfaces, reference
      resolution for hooks/skills/agents, and absence of excluded state

**Real-world example**: A Trellis Codex harness export initially copied only
`.codex/` and selected `.trellis/` content. Inspection of the published init
writer graph plus a fresh init showed two additional surfaces,
`.agents/skills/` and `AGENTS.md`, as well as an empty `.codex/skills/` root,
managed metadata, tests, and default state skeletons. A writer-to-output
manifest and set/hash validation converted that implicit boundary into an
auditable export contract.
