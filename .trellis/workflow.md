...
6. **Coordinate, don't implement** — the Sol/medium main session delegates every active-task execution unit to a Trellis sub-agent
...
Phase 2: Execute → dispatch execution only after task status is in_progress
...
Flow: `trellis-implement` -> one paired `trellis-check` stage per unit -> optional spec-update unit and paired check -> read-only `trellis-final-check` -> commit (Phase 3.5) -> `/trellis:finish-work`.
Main session: coordination-only. It must dispatch every substantive implementation, configuration, test, documentation, or spec change; it may only plan, split units, dispatch, verify evidence, integrate, update task state, and commit. Run no more than three independent workers concurrently, never with overlapping write ownership; serialize dependencies.
Codex-only persistent routes: Sol/medium is coordination-only and dispatches `trellis-research` at Terra/medium for missing evidence. Classify every implementation unit as `default` or `hard`, then immediately before native spawn invoke `resolve_dispatch_lane(root, "implement", lane)` for `dispatch:implement:<lane>` against `.codex/trellis-dispatch.toml` and pass the returned exact `model` and `effort` as the spawn's `model` and `reasoning_effort` overrides. After implementation, invoke the resolver again for the matching `dispatch:check:<lane>`. The per-turn `<trellis-dispatch>` context makes all four lanes visible but is not a cached substitute for that immediate resolution; a BLOCKED lane prohibits only that lane's spawn until repaired, with no prose or inherited-model fallback. Use the matching Sol assurance role for debugging, review, audit, or release gates. Claude Code retains its native `trellis-research`, `trellis-implement`, and `trellis-check` dispatch path; it does not require Codex model names or the four Codex assurance profiles.
Check handoff: only completion of an implementation unit triggers its paired check. The checker fixes every in-scope finding and reruns validation in that same stage; checker fixes never trigger another `trellis-implement` or `trellis-check` dispatch.
...
Flow: `trellis-before-dev` -> edit -> one inline `trellis-check` stage per unit -> optional `trellis-update-spec` unit and paired check -> read-only final gate -> commit (Phase 3.5) -> `/trellis:finish-work`.
Do not dispatch implement/check sub-agents in inline mode.
Inline check fixes do not restart implementation or trigger another check stage.
...
- 3.4 Final gate `[required · once]`
- 3.5 Commit changes `[required · once]`
- 3.6 Wrap-up reminder

> Note: step 3.1 remains reserved for compatibility. Final readiness is a distinct read-only gate after spec sync.
...
Goal: coordinate dispatched workers that turn reviewed planning artifacts into code that passes quality checks.

#### 2.1 Implement `[required · repeatable]`

The main session must delegate this work. Split the task into
independent units before dispatch when their owned paths do not overlap; run at
most three such units concurrently. On Codex, classify complex or cross-boundary
units as `hard`; otherwise classify them as `default`. Immediately before native
spawn, resolve the matching persisted implement lane and pass its exact override.
Other platforms use their native implementation agent without requiring a Codex route.
...
- **Codex route**: classify `default` or `hard`, invoke `resolve_dispatch_lane(root, "implement", lane)` immediately before spawn (the `<trellis-dispatch>` block is visibility, not cached authority), and pass its returned exact `model` / `effort` as `model` / `reasoning_effort`; Claude Code uses its native `trellis-implement` agent without a Codex model requirement
- **Task description**: Implement the reviewed task artifacts exactly; do not run validation commands
...
- **Agent type**: `trellis-implement`
- **Route**: `bounded_worker` (`gpt-5.6-terra` / `high`)
- **Task description**: Implement the reviewed task artifacts exactly; do not run validation commands
...
5. Hand off the changed files and prescribed-action result to the paired check

[/codex-inline, Kilo, Antigravity, Devin]

#### 2.2 Quality check `[required · repeatable]`

Every completed implementation unit requires one successful `trellis-check`
stage before the main session can integrate the result or proceed to finish.
In sub-agent mode, completion of `trellis-implement` is the only normal trigger
for its paired check dispatch. In inline mode, completion of the direct edit is
the trigger for the inline check skill. Research, planning, or fixes made by a
checker do not trigger a check stage.
On Codex, use the same `default` or `hard` classification as implementation,
resolve the matching persisted check lane immediately before native spawn, and
pass its exact override. Claude Code uses its native `trellis-check` agent without a Codex model or reasoning-effort requirement.
...
- **Codex route**: invoke `resolve_dispatch_lane(root, "check", lane)` for the matching lane immediately before spawn (the `<trellis-dispatch>` block is visibility, not cached authority), and pass its returned exact `model` / `effort` as `model` / `reasoning_effort`; Claude Code uses its native `trellis-check` agent without a Codex model requirement
- **Task description**: Audit the named unit and run only `check.md` paired commands within its explicit budget
...
- Fix every finding within the assigned task/path authority directly, regardless
  of whether the fix is mechanical or substantive
- Run only commands explicitly listed in `check.md`, respecting tier and rerun limits
- Report only blockers that require a missing product decision, new authority,
  or paths outside the assigned scope

The main session must not dispatch `trellis-implement` again for findings from
the checker and must not dispatch another checker because the checker edited
files. The same check stage owns its fix-and-revalidate loop. A terminated or
failed checker may be retried as the same stage; that is not a new role handoff.

[/Claude Code, Cursor, OpenCode, codex-sub-agent, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi, Oh My Pi, ZCode, Snow, Reasonix, Trae, Grok, Kimi Code]

[codex-inline, Kilo, Antigravity, Devin]

Load the `trellis-check` skill and verify the named unit per `check.md`:
- Protected baseline and ownership
- Requirement and self-fix scope
- Only listed command evidence

If issues are found, fix them directly and rerun validation inside the same
inline check stage until green. Do not restart implementation or invoke a
second check stage because the checker changed files.

[/codex-inline, Kilo, Antigravity, Devin]

No paired check may infer a broader validation tier from task size or final-stage position. The read-only final gate decides readiness only from the frozen candidate and `check.md` finalGate budget.

#### 2.3 Rollback `[on demand]`

- `check` reveals a missing product decision or PRD defect that it cannot
  safely resolve → report the blocker and return to Phase 1. Do not
  automatically bounce from check to implement; only a newly reviewed change
  creates a later implementation unit.
...
The main session reviews whether this task produced new knowledge worth
recording. In sub-agent mode, it must not edit the spec itself:
- Newly discovered patterns or conventions
- Pitfalls you hit
- New technical decisions

When an update is needed, sub-agent mode dispatches `trellis-implement` with
ownership limited to the relevant `.trellis/spec/` paths and instructs it to
use `trellis-update-spec`; inline mode applies `trellis-update-spec` as a direct
edit. Either path is a new implementation unit, so completion triggers exactly
one paired `trellis-check` stage (the inline skill in inline mode). Even if the
conclusion is "nothing to update", record the judgment in the task evidence.

#### 3.4 Final gate `[required · once]`

Freeze the candidate with `task.py freeze-candidate <task>`, then dispatch the
read-only `trellis-final-check` role. It receives check-family context and may
run only finalGate commands in `check.md` (zero by default). It returns `ready`
or `blocked` and never edits files. A blocked result requires a new explicit
implementation unit and paired check before another final-gate attempt.
